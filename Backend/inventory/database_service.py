...
class InventoryDatabaseService:
    ...
    def create_tables(self):
        ...
        try:
            ...
            if count == 0:
                ...
                print("[OK] Warehouses inserted: Mumbai and Bhopal")
            else:
                print("[INFO] Warehouses already exist — skipping insert")
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS inventory_master (
                    ...
                    supplier_location VARCHAR(100)  NOT NULL DEFAULT 'Mumbai',
            ...
            conn.commit()
            print("[OK] Inventory tables created successfully in chainlink_pro")

            # ── Migrate: Add supplier_location column if it doesn't exist ────
            try:
                cursor.execute('''
                    ALTER TABLE inventory_master 
                    ADD COLUMN supplier_location VARCHAR(100) NOT NULL DEFAULT 'Mumbai'
                ''')
                conn.commit()
                print("[OK] Added supplier_location column to inventory_master")
            except Exception as e:
                # Column already exists or other error
                if "Duplicate column" in str(e):
                    print("[INFO] supplier_location column already exists")
                else:
                    print(f"[WARN] Migration note: {e}")

        except Exception as e:
            print(f"[ERROR] Error creating inventory tables: {e}")
            conn.rollback()
        finally:
            cursor.close()
            conn.close()
            ...
            cursor.execute('''
                INSERT INTO inventory_master
                    (product_id, warehouse_id, current_stock,
                     lead_time, supplier_name, supplier_location)
                VALUES (%s, %s, %s, %s, %s, %s)
                ON DUPLICATE KEY UPDATE
                    current_stock      = VALUES(current_stock),
                    lead_time          = VALUES(lead_time),
                    supplier_name      = VALUES(supplier_name),
                    supplier_location  = VALUES(supplier_location)
            ''', (product_id, warehouse_id, current_stock,
                  lead_time, supplier_name, supplier_location))
            conn.commit()
            return True
    ...
    def get_inventory_by_warehouse(self, warehouse_id):
        ...
        try:
            ...
            print(f"[ERROR] Error fetching inventory: {e}")
            return []
        finally:
            cursor.close()
            conn.close()
    ...
    def get_warehouse(self, warehouse_id):
        ...
        try:
            ...
            print(f"[ERROR] Error fetching warehouse: {e}")
            return None
        finally:
            cursor.close()
            conn.close()
    ...
    def get_all_warehouses(self):
        ...
        try:
            ...
            print(f"[ERROR] Error fetching warehouses: {e}")
            return []
        finally:
            cursor.close()
            conn.close()
