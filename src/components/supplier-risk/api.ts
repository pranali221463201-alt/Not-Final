...
// Major Indian manufacturing hubs for plastic/materials suppliers
const SUPPLIER_LOCATIONS = [
  'Mumbai, Maharashtra',      // Financial & logistics hub - warehouse location
  'Pune, Maharashtra',        // Manufacturing hub
  'Nagpur, Maharashtra',      // Industrial hub in central India
  'Bhopal, Madhya Pradesh',   // Warehouse location
  'Indore, Madhya Pradesh',   // Major manufacturing hub in central India
  'Ahmedabad, Gujarat',       // Major plastics hub
  'Surat, Gujarat',           // Textile & plastics
  'Vadodara, Gujarat',        // Plastics manufacturing
  'Bangalore, Karnataka',     // Tech & manufacturing
  'Hyderabad, Telangana',     // Growing manufacturing
];

// Intelligently assign location based on supplier name hash
const assignSupplierLocation = (supplierName: string): string => {
  // Hash the supplier name to get consistent location assignment
  let hash = 0;
  ...
  const index = Math.abs(hash) % SUPPLIER_LOCATIONS.length;
  return SUPPLIER_LOCATIONS[index];
};

const mapSupplier = (item: SupplierRanking): SupplierRiskRow => {
  const location = assignSupplierLocation(item.supplier_name);
...
export const fetchSupplierMonitoringData = async (): Promise<SupplierRiskRow[]> => {
  ...
  try {
    ...
    return (suppliers ?? []).map((supplier: any) => ({
      ...
      location: assignSupplierLocation(supplier.company_legal_name || `Supplier ${supplier.supplier_id}`),
      industry: 'Materials',
      connectedMaterials: [],
